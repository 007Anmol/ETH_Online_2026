-- Team 1 + shared enums (names must stay aligned with on-chain / app usage).
-- Idempotent: safe if Team 1 already created these types.
...
do $$ begin
  create type product_status as enum (
    'CREATED', 'MINTED', 'TAG_PENDING', 'TAG_BOUND', 'READY_FOR_DISPATCH',
    'IN_TRANSIT', 'SUSPECT_COUNTERFEIT',
    'AUTHENTIC', 'SOLD', 'OWNED', 'RESALE_INITIATED',
    'PHYSICAL_HANDOFF_PENDING', 'NEW_OWNER_CONFIRMED',
    'REVOKED'
  );
exception when duplicate_object then null;
end $$;
...
do $$ begin
  create type checkpoint_type as enum (
    'FACTORY', 'DISTRIBUTOR', 'LOGISTICS_HUB', 'RETAILER', 'CONSUMER_POINT_OF_SALE'
  );
exception when duplicate_object then null;
end $$;
