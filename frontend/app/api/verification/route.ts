...
export async function GET(request: Request) {
  try {
    ...
    const [shipments, anomalies] = await Promise.all([
      supabaseRequest<unknown[]>(`shipments?product_id=eq.${encodeURIComponent(productId)}&order=created_at.desc`),
      supabaseRequest<unknown[]>(`anomalies?product_id=eq.${encodeURIComponent(productId)}&order=created_at.desc&limit=1`),
    ]);
