/**
 * Same fixture pattern as seed-demo-product-for-wallet.ts, for the new
 * seller account (Account 1) picked after the earlier account-switching
 * issues in MetaMask. Isolated batch/product code — doesn't touch the
 * VC-HARSHEEL-QA-000001 fixture or any other seed/test data.
 *
 * Run with: npx tsx scripts/seed-demo-product-account1.ts
 */
...
const BATCH_CODE = "MANUAL-QA-ACCOUNT1-001";
const PRODUCT_CODE = "VC-QA-ACCOUNT1-000001";
const SERIAL_NUMBER = "QA-ACCOUNT1-SN-000001";
...
async function main() {
    ...
    .upsert(
      {
        ...
        product_name: "VeriChain Manual QA Fixture (Account 1)",
  ...
  if (existingOwnership?.owner_wallet_address === SELLER_WALLET_ADDRESS) {
    console.log("Ownership record already points at Account 1 — nothing to mint.");
    return;
  }
  ...
  console.log("Fixture ready: Account 1 now owns a real Hedera-minted product.");
}
