...
import type { TelemetryPoint } from "./types/checkpoint";

export const EARTH_RADIUS_KM = 6371;
export const MAX_TRANSPORT_KMH = 950;
...
export function evaluateTelemetry(
  ...
  const findings: Array<{ points: number; message: string }> = [];
  ...
  if (current.expectedRoute?.length && current.actualRoute?.length) {
    ...
    if (routeChanged) {
      findings.push({ points: 30, message: "Observed checkpoints differ from the configured route." });
    }
  }

  if (recent.some((point) => point.checkpoint === current.checkpoint && Math.abs(point.timestamp - current.timestamp) <= 1800)) {
    findings.push({ points: 20, message: "The product appeared at this checkpoint again within 30 minutes." });
  }

  const riskScore = Math.min(100, findings.reduce((score, finding) => score + finding.points, 0));
  return {
